            ...
            {cards.map((c, i) => (
              <div key={i} className="bg-white p-5 border border-[#DFDFDF] rounded-[4px] shadow-sm flex flex-col">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-[14px] font-bold text-[#484848]" style={{ fontFamily: 'Noto Sans, sans-serif' }}>{c.label}</span>
                </div>
                <div className="text-[24px] font-bold text-[#111111] mt-auto" style={{ fontFamily: 'Noto Sans, sans-serif' }}>
                  {c.value}
                </div>
              </div>
            ))}
          ...
          {stats && stats.lowStockProducts.length > 0 && (
            <div className="bg-white border border-[#DFDFDF] rounded-[4px] shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-[#DFDFDF] flex items-center justify-between">
                <h3 className="text-[16px] font-bold text-[#111111]" style={{ fontFamily: 'Noto Sans, sans-serif' }}>
                  Sản phẩm sắp hết hàng
                </h3>
                <Link to="/admin/products" className="text-[13px] font-bold text-[#003399] hover:underline" style={{ fontFamily: 'Noto Sans, sans-serif' }}>
                  Xem tất cả
                </Link>
              </div>
                  ...
                  <tbody>
                    {stats.lowStockProducts.map(p => (
                      <tr key={p._id} className="border-b border-[#DFDFDF] hover:bg-[#F5F5F5] transition-colors">
                        <td className="px-4 py-3 text-[14px] font-bold text-[#111111]" style={{ fontFamily: 'Noto Sans, sans-serif' }}>{p.name}</td>
                        <td className="px-4 py-3 text-[14px] text-[#484848]" style={{ fontFamily: 'Noto Sans, sans-serif' }}>{fmt(p.price)}₫</td>
